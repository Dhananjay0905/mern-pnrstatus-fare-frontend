...
const PNRStatusPage = () => {
    ...
    const fetchPNRStatus = async () => {
        try {
            const response = await axios.get(`https://mern-pnrstatus-fare-server.vercel.app/pnr/${pnr}`);
        ...
        if (status && status.data) {
            ...
            if (data.PassengerStatus) {
                ...
                data.PassengerStatus.forEach((passenger, index) => {
                    doc.text(`Passenger Number: ${passenger.Number}`, 10, 190 + (index * 10));
                    doc.text(`Booking Status: ${passenger.BookingStatus}`, 10, 200 + (index * 10));
                    doc.text(`Current Status: ${passenger.CurrentStatus}`, 10, 210 + (index * 10));
                });
            }
        }
